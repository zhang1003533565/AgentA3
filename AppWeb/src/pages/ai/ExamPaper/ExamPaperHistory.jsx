...
function ExamPaperHistory({ refreshKey = 0 }) {
  ...
  const publishingIdsRef = useRef(new Set())
  ...
  const handlePublishChange = async (record) => {
    if (publishingIdsRef.current.has(record.id)) return
    const action = getExamPaperPublishAction(record.published)
    ...
    try {
      ...
      publishingIdsRef.current.delete(record.id)
      setPublishingIds(new Set(publishingIdsRef.current))
    }
  }
  ...
  const columns = [
    ...
    { title: '总分', dataIndex: 'totalScore', width: 90, render: (value) => `${value ?? 0} 分` },
    {
      ...
      render: (published) => {
        const state = getExamPaperPublishState(published)
    ...
    {
      title: '操作', fixed: 'right', width: 440,
      render: (_, record) => {
        const action = getExamPaperPublishAction(record.published)
        const actionButton = (
          <Button
            danger={record.published}
            icon={record.published ? <StopOutlined /> : <CloudUploadOutlined />}
            loading={publishingIds.has(record.id)}
            onClick={record.published ? undefined : () => handlePublishChange(record)}
          >{action.label}</Button>
        )
        return (
          <Space className="exam-paper-actions" wrap>
            ...
            {record.published ? (
              <Popconfirm
                ...
                okText="确认取消"
                cancelText="保留发布"
                onConfirm={() => handlePublishChange(record)}
              >
                {actionButton}
              </Popconfirm>
            ) : actionButton}
  ...
  const detailPublishState = getExamPaperPublishState(detail?.published)
          ...
          <Space direction="vertical" size="large" className="exam-paper-detail">
            <Descriptions bordered column={{ xs: 1, sm: 2 }}>
              ...
              <Descriptions.Item label="发布状态">
                <Tag color={detailPublishState.color}>{detailPublishState.label}</Tag>
              </Descriptions.Item>
              <Descriptions.Item label="发布时间">{formatTime(detail.publishTime)}</Descriptions.Item>
