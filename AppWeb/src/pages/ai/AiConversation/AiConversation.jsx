          ...
          {detailLoading ? <Spin /> : messages.length ? messages.map((item) => (
            <div key={item.id} className={`ai-conversation-message ai-conversation-message--${item.role}`}>
              ...
              <div className="ai-conversation-bubble">
                ...
                {item.attachments?.length ? <div className="ai-conversation-attachments">{item.attachments.map((attachment, attachmentIndex) => <div className="ai-conversation-attachment" key={`${item.id}-attachment-${attachmentIndex}`}><FileOutlined /><span title={attachmentName(attachment)}>{attachmentName(attachment)}</span><div className="ai-conversation-attachment-actions"><Button type="text" size="small" icon={<EyeOutlined />} onClick={() => openAttachment(attachment)}>打开</Button><Button type="text" size="small" icon={<DownloadOutlined />} onClick={() => downloadAttachment(attachment)}>下载</Button></div></div>)}</div> : null}
              </div>
