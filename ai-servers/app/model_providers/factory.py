...
_provider_cache: Dict[tuple[str, ...], ChatModelProvider] = {}
...
def build_chat_model_provider(config) -> ChatModelProvider:
    ...
    raise RuntimeError(f"暂不支持的模型服务商: {config.provider}")
...
def get_chat_model_provider() -> ChatModelProvider:
    ...
    cache_key = (
        *config.cache_key(),
        provider_cache_key(),
        str(get_active_llm_timeout_seconds()),
        str(get_active_max_output_tokens() or ""),
    )
