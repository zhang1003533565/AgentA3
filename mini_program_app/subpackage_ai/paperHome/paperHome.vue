  ...
  <view class="page">
    ...
    <scroll-view scroll-y class="scroll">
      ...
      <view v-for="paper in papers" :key="paper.id" class="swipe-row">
        <view v-if="paper.status === 'draft'" class="delete-action" @click.stop="confirmDelete(paper)">{{ deletingPaperId === paper.id ? '删除中' : '删除' }}</view>
      ...
      <view v-if="!loading && loadError" class="empty error-state" @click="load"><text>试卷数据加载失败</text><text>{{ loadError }}，点击重新加载</text></view>
      <view v-else-if="!loading && papers.length === 0" class="empty"><text>还没有试卷</text><text>点击上方创建你的第一份试卷</text></view>
    </scroll-view>
  </view>
</template>
...
export default {
  ...
  data() {
    return {
      papers: [], loading: false, loadError: '', swipedPaperId: null, touchStartX: 0, deletingPaperId: null,
      sources: [
  ...
  onShow() { this.load() },
  methods: {
    async load() { this.loading = true; this.loadError = ''; try { const result = await listPapers({ status: 'draft' }); this.papers = result.data || [] } catch (error) { this.loadError = error?.data?.msg || error?.msg || '请检查后端服务' } finally { this.loading = false } },
    ...
    confirmDelete(paper) {
      if (this.deletingPaperId !== null) return
      uni.showModal({ title: '确认删除？', content: '删除后该草稿无法恢复。', cancelText: '取消', confirmText: '删除', confirmColor: '#df6565', success: result => { if (result.confirm) this.removePaper(paper) } })
    },
    async removePaper(paper) {
      this.deletingPaperId = paper.id
      try {
        await deletePaper(paper.id)
        this.papers = this.papers.filter(item => item.id !== paper.id)
        this.swipedPaperId = null
        uni.showToast({ title: '删除成功', icon: 'success' })
      } catch (error) {
        const message = error?.data?.msg || error?.msg || error?.message || '删除失败，请检查后端服务'
        uni.showToast({ title: message, icon: 'none' })
      } finally {
        this.deletingPaperId = null
      }
    }
  }
}
