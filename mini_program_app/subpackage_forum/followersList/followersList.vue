<template>
  <view class="followers-container">
    <nav-bar title="粉丝列表" :showBack="true" fixed placeholder />
    <view class="followers-content">
      <view v-if="list.length" class="followers-list">
        <view
          v-for="item in list"
          :key="item.userId"
          ...
          @click="goToUser(item)"
        >
          <image class="followers-avatar" :src="item.avatar || '/static/logo.png'" mode="aspectFill" />
          <view class="followers-info">
            <text class="followers-name">{{ item.userName }}</text>
            <text class="followers-desc">{{ item.isFollowing ? '已关注' : '未关注' }}</text>
          </view>
          <view class="followers-action" @click.stop="toggleFollow(item)">
      ...
      <view v-else class="empty">
        ...
        />
        <text class="empty-desc">还没有粉丝</text>
      </view>
    </view>
  </view>
</template>
...
import { getFollowers, getMyFollowers, toggleFollowUser } from '@/api/forum.js'
    ...
    async loadList() {
      try {
        ...
        const res = this.userId
          ? await getFollowers(this.userId, params)
          : await getMyFollowers(params)
        const records = res?.data?.records || []
        this.list = records
          .map((item) => {
            const id = item.userId || item.followerId || item.fansId
            return {
              userId: id,
