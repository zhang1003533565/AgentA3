...
test('PPT options are requested once and reused from local cache', async () => {
  ...
  `)(async options => {
    ...
    return { data: {
      ...
      templates: [{ id: 'general', name: '简约通用', layouts: [{}] }],
...
test('PPT options keep using an expired complete cache when the server is unavailable', async () => {
  const storage = new Map([
    ['aiPptOptions:v8', {
      data: {
  ...
  const api = new Function('request', 'streamSse', 'BASE_URL', 'PPT_OPTIONS_BYPASS_CACHE', 'getToken', 'uni', `${source}
    return { getPptOptions }
  `)(async () => { throw new Error('server timeout') }, () => {}, '', false, () => '', uni)

  const result = await api.getPptOptions()

  assert.equal(result.templates[0].id, 'general')
})
...
test('PPT template thumbnails are deduplicated and kept in a persistent local cache', async () => {
  const downloads = []
  const storage = new Map()
  const uni = {
    getStorageSync: key => storage.get(key),
    ...
    downloadFile: options => {
      ...
      options.success({ statusCode: 200, tempFilePath: '/tmp/template-thumb.png' })
    },
    saveFile: options => options.success({ savedFilePath: '/saved/template-thumb.png' })
  }
  const source = readFileSync(join(__dirname, '../api/ppt.js'), 'utf8')
    .replace(/^import .*$/gm, '')
    .replace(/export const /g, 'const ')
    .replace(/export function /g, 'function ')
  const api = new Function('request', 'streamSse', 'BASE_URL', 'PPT_OPTIONS_BYPASS_CACHE', 'getToken', 'uni', `${source}
    return { downloadPptTemplateThumbnail, clearPptTemplateThumbnailCache }
  `)(async () => ({}), () => {}, 'https://example.test', false, () => 'token', uni)

  const [first, second] = await Promise.all([
    api.downloadPptTemplateThumbnail('modern'),
    api.downloadPptTemplateThumbnail('modern')
  ])

  assert.equal(first, '/saved/template-thumb.png')
  assert.equal(second, first)
  assert.equal(downloads.length, 1)
  assert.equal(storage.get('aiPptTemplateThumbnail:v2:modern'), first)

  api.clearPptTemplateThumbnailCache('modern')
  await api.downloadPptTemplateThumbnail('modern')
