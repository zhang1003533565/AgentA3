...
test('AI Create keeps the existing visible campus tools and their order', () => {
  ...
  assert.match(page, /实践报告[\s\S]*课程报告[\s\S]*英语作文[\s\S]*活动总结[\s\S]*学科出题[\s\S]*题库生成[\s\S]*学习计划[\s\S]*考研题目[\s\S]*文章主题大纲[\s\S]*雅思大作文[\s\S]*思想汇报/)
...
test('every visible AI Create action resolves to a real page and never silently returns', async () => {
  ...
  const visibleNames = [
    ...
    '实践报告', '课程报告', '英语作文', '活动总结', '学科出题', '题库生成', '学习计划', '考研题目', '文章主题大纲', '雅思大作文', '思想汇报',
