...
public class PaperService {
    ...
    @Transactional public PaperDTO.BankVO saveBank(PaperDTO.BankRequest req, Long userId, Long id) {
        ...
        b.setName(req.getName().trim()); b.setSubjectId(req.getSubjectId()); b.setDescription(req.getDescription()); b.setBankType(req.getBankType()); b.setVisibility("private"); b.setOwnerId(userId);
    ...
    @Transactional public void addToBank(Long bankId, Long questionId, Long userId) { QuestionBank b=bankRepo.findById(bankId).orElseThrow(()->new BusinessException(Result.NOT_FOUND_CODE,"题库不存在")); if(!Objects.equals(b.getOwnerId(),userId)) throw new BusinessException(Result.FORBIDDEN_CODE,"无权操作该题库"); questionRepo.findById(questionId).orElseThrow(()->new BusinessException(Result.NOT_FOUND_CODE,"题目不存在")); if(bankItemRepo.findByBankIdAndQuestionId(bankId,questionId).isPresent()) throw new BusinessException(Result.BAD_REQUEST_CODE,"题目已在该题库中"); QuestionBankItem item=new QuestionBankItem();item.setBankId(bankId);item.setQuestionId(questionId);item.setAddedBy(userId);bankItemRepo.save(item); }
    ...
    public List<PaperDTO.QuestionVO> questions(Long bankId, Long userId, String keyword, String type, String difficulty, Long paperId, boolean favorites) {
        List<Question> qs;
        if (favorites) qs = favoriteRepo.findByUserIdOrderByCreateTimeDesc(userId).stream().map(f -> questionRepo.findById(f.getQuestionId()).orElse(null)).filter(Objects::nonNull).toList();
        else if (bankId == null) qs = questionRepo.findAll();
        else { QuestionBank b=bankRepo.findById(bankId).orElseThrow(()->new BusinessException(Result.NOT_FOUND_CODE,"题库不存在")); if("private".equals(b.getVisibility())) qs=bankItemRepo.findByBankIdOrderByCreateTimeDesc(bankId).stream().map(x->questionRepo.findById(x.getQuestionId()).orElse(null)).filter(Objects::nonNull).toList(); else qs=questionRepo.findByBankIdOrderByIdDesc(bankId); }
    ...
    @Transactional public boolean toggleFavorite(Long questionId, Long userId, boolean add) { Question q = questionRepo.findById(questionId).orElseThrow(() -> new BusinessException(Result.NOT_FOUND_CODE, "题目不存在")); Optional<QuestionFavorite> old = favoriteRepo.findByUserIdAndQuestionId(userId, q.getId()); if (add && old.isEmpty()) { QuestionFavorite f = new QuestionFavorite(); f.setUserId(userId); f.setQuestionId(q.getId()); favoriteRepo.save(f); } else if (!add) old.ifPresent(favoriteRepo::delete); return add; }
    ...
    @Transactional public PaperDTO.PaperVO addQuestion(Long paperId, PaperDTO.QuestionRequest req, Long userId) { Paper p = ownPaper(paperId, userId); if (!"draft".equals(p.getStatus())) throw new BusinessException(Result.BAD_REQUEST_CODE, "已完成试卷不能修改"); questionRepo.findById(req.getQuestionId()).orElseThrow(() -> new BusinessException(Result.NOT_FOUND_CODE, "题目不存在")); if (paperQuestionRepo.findByPaperIdAndQuestionId(paperId, req.getQuestionId()).isPresent()) throw new BusinessException(Result.BAD_REQUEST_CODE, "题目已加入试卷"); List<PaperQuestion> all = paperQuestionRepo.findByPaperIdOrderByQuestionOrderAsc(paperId); PaperQuestion pq = new PaperQuestion(); pq.setPaperId(paperId); pq.setQuestionId(req.getQuestionId()); pq.setQuestionOrder(req.getQuestionOrder() == null ? all.size() + 1 : req.getQuestionOrder()); pq.setScore(req.getScore() == null || req.getScore() <= 0 ? 5 : req.getScore()); pq.setSourceType(req.getSourceType()); pq.setSourceId(req.getSourceId()); paperQuestionRepo.save(pq); return paperVO(p, userId); }
    ...
    @Transactional public PaperDTO.PaperVO removeQuestion(Long paperId, Long questionId, Long userId) { Paper p = ownPaper(paperId, userId); paperQuestionRepo.deleteByPaperIdAndQuestionId(paperId, questionId); return paperVO(p, userId); }
    ...
    @Transactional public void deletePaper(Long id, Long userId) { Paper p = ownPaper(id, userId); paperQuestionRepo.deleteAll(paperQuestionRepo.findByPaperIdOrderByQuestionOrderAsc(id)); paperRepo.delete(p); }
