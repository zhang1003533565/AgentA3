...
public class PaperService {
    ...
    public List<PaperDTO.QuestionVO> questions(Long bankId, Long userId, String keyword, String type, String difficulty, Long paperId, boolean favorites) {
        ...
        else { QuestionBank b=bankRepo.findById(bankId).orElseThrow(()->new BusinessException(Result.NOT_FOUND_CODE,"题库不存在")); if(!"public".equals(b.getVisibility())) { if(!Objects.equals(b.getOwnerId(),userId)) throw new BusinessException(Result.FORBIDDEN_CODE,"无权查看该题库"); qs=bankItemRepo.findByBankIdOrderByCreateTimeDesc(bankId).stream().map(x->questionRepo.findById(x.getQuestionId()).orElse(null)).filter(Objects::nonNull).toList(); } else qs=questionRepo.findByBankIdOrderByIdDesc(bankId); }
    ...
    public PaperDTO.QuestionVO questionDetail(Long questionId, Long userId, Long paperId) {
        Question q = questionRepo.findById(questionId).orElseThrow(() -> new BusinessException(Result.NOT_FOUND_CODE, "题目不存在"));
    ...
    @Transactional public void deletePaper(Long id, Long userId) { Paper p = ownPaper(id, userId); paperLayoutRepo.deleteByPaperId(id); paperQuestionRepo.deleteAll(paperQuestionRepo.findByPaperIdOrderByQuestionOrderAsc(id)); paperRepo.delete(p); }
