...
对学生和教师而言，当前 App 入口承载学习助手、会议、资源与考试等获授权交互。MaxKB 与 AI 题目生成控制器当前只允许 ADMIN；向教师开放相应 Web 教学治理能力属于设计目标。Web 同时允许 MERCHANT 登录，但路由和导航未按角色过滤，试卷创建、列表、详情与预览也只校验登录，因此不能把商户入口描述为已经与教学域隔离；该授权缺口需要后续加固。对项目评审而言，需求状态和证据路径使功能演示能够回到可核验的工程事实。
